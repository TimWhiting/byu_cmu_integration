...
class gamesBf:
    def __init__(self, gameStr, me):
        ...
        self.name="BF"
        #should be probability of the agent being selected
       ...
       # self.baseline=np.argmax(self.priorsOnly)
        self.baseline=max(self.priorsOnly)
...
class gamesBfAAT(gamesBf):
    def __init__(self, estimation_model, output_assumptions, name, me=0, gameStr="blocks2"):
        gamesBf.__init__(self, gameStr, me)

        self.output_assumptions = output_assumptions
        self.alignment_profile = []  # what is this storing
        #print("here", self.alignment_profile)
    ...
    def obtain_performance_features(self, printflag=False):
        ...
        baseline_estimation = self.baseline
        # is baseline estimation just the probability value of the most probable model?
        # MLE,confidence
        # rounds so far, and avg acc till now,
        #
        ...
        if printflag:
            print("baseline:", baseline_estimation, "||| number of rounds:", feature_1,
                  "||| average accuracy", round(feature_2, 3))

        return [float(baseline_estimation), feature_1, feature_2]

    def _add_to_profile(self, partnerAction, bfAction):
        profile_item = []

        # output assumptions are lambda state, action, result: true/false

        for a in self.output_assumptions:
            profile_item.append(int(a(partnerAction, bfAction)))

        self.alignment_profile.append(profile_item)
        #print(self.alignment_profile)
